...
SWARM_STATIC=$(gcloud compute addresses list --filter "NAME=swarm-ucp" --format "table[no-heading](NAME)")
if [ -z "$SWARM_STATIC" ]; then
    gcloud compute addresses create swarm-ucp --region $REGION
fi
SWARM_IP=$(gcloud compute addresses describe swarm-ucp --region $REGION --format "value(ADDRESS)")
...
gcloud beta compute instances create swarm-m \
    ...
    --metadata docker-install-status="pending",startup-script="$(cat ucp_setup.sh)",docker-ee-url="${DOCKER_EE_URL}",docker-ee-lic="${DOCKER_EE_LIC}" \
